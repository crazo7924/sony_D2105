                            ...
                            TVariableInfoList& infoList)
{
    if (type.getBasicType() == EbtStruct) {
        if (type.isArray()) {
            ...
            getUserDefinedVariableInfo(type, name, mappedName, infoList);    //SOMC KK PATCH Patches_w16.5
        }
    } else {
        getBuiltInVariableInfo(type, name, mappedName, infoList);    //SOMC KK PATCH Patches_w16.5
    }
}
                            ...
                            TVariableInfoList& infoList)
{
    ...
    if (type.isArray()) {
        ...
        varInfo.mappedName = (mappedName + "[0]").c_str();    //SOMC KK PATCH Patches_w16.5
        varInfo.size = type.getArraySize();
    } else {
        varInfo.name = name.c_str();
        varInfo.mappedName = mappedName.c_str();    //SOMC KK PATCH Patches_w16.5
        varInfo.size = 1;
    }
