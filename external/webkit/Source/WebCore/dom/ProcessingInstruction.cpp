...
void ProcessingInstruction::checkStyleSheet()
{
    if (m_target == "xml-stylesheet" && document()->frame() && parentNode() == document()) {
        ...
        if (href.length() > 1 && href[0] == '#') {
            ...
            ResourceRequest request(document()->completeURL(href));    //SOMC KK PATCH Patches_w16.5
#if ENABLE(XSLT)
            if (m_isXSL)
                m_cachedSheet = document()->cachedResourceLoader()->requestXSLStyleSheet(request);    //SOMC KK PATCH Patches_w16.5
            else
#endif
            {
                ...
                m_cachedSheet = document()->cachedResourceLoader()->requestCSSStyleSheet(request, charset);    //SOMC KK PATCH Patches_w16.5
            }
            if (m_cachedSheet)
