...
void CSSImportRule::insertedIntoParent()
{
    ...
    ResourceRequest request(parentSheet->document()->completeURL(absHref));    //SOMC KK PATCH Patches_w16.5
    if (parentSheet->isUserStyleSheet())
        m_cachedSheet = cachedResourceLoader->requestUserCSSStyleSheet(request, parentSheet->charset());    //SOMC KK PATCH Patches_w16.5
    else
        m_cachedSheet = cachedResourceLoader->requestCSSStyleSheet(request, parentSheet->charset());    //SOMC KK PATCH Patches_w16.5
    if (m_cachedSheet) {
