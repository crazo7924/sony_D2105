...
static CachedResource* createResource(CachedResource::Type type, ResourceRequest& request, const String& charset)    //SOMC KK PATCH Patches_w16.5
{
    switch (type) {
    case CachedResource::ImageResource:
        return new CachedImage(request);    //SOMC KK PATCH Patches_w16.5
    case CachedResource::CSSStyleSheet:
        return new CachedCSSStyleSheet(request, charset);    //SOMC KK PATCH Patches_w16.5
    case CachedResource::Script:
        return new CachedScript(request, charset);    //SOMC KK PATCH Patches_w16.5
    case CachedResource::FontResource:
        return new CachedFont(request);    //SOMC KK PATCH Patches_w16.5
#if ENABLE(XSLT)
    case CachedResource::XSLStyleSheet:
        return new CachedXSLStyleSheet(request);    //SOMC KK PATCH Patches_w16.5
#endif
...
CachedImage* CachedResourceLoader::requestImage(ResourceRequest& request)    //SOMC KK PATCH Patches_w16.5
{
    if (Frame* f = frame()) {
    ...
    CachedImage* resource = static_cast<CachedImage*>(requestResource(CachedResource::ImageResource, request, String()));    //SOMC KK PATCH Patches_w16.5
    if (resource) {
#ifdef ANDROID_BLOCK_NETWORK_IMAGE
        resource->setAutoLoadWasPreventedBySettings(!autoLoadImages() || shouldBlockNetworkImage(request.url()));    //SOMC KK PATCH Patches_w16.5
#else
        resource->setAutoLoadWasPreventedBySettings(!autoLoadImages());
#endif
        if (autoLoadImages() && resource->stillNeedsLoad()) {
#ifdef ANDROID_BLOCK_NETWORK_IMAGE
            if (shouldBlockNetworkImage(request.url())) {    //SOMC KK PATCH Patches_w16.5
                return resource;
            }
#endif
...
CachedCSSStyleSheet* CachedResourceLoader::requestUserCSSStyleSheet(ResourceRequest& request, const String& charset)    //SOMC KK PATCH Patches_w16.5
{
    KURL url = MemoryCache::removeFragmentIdentifierIfNeeded(request.url());    //SOMC KK PATCH Patches_w16.5
...
CachedResource* CachedResourceLoader::requestResource(CachedResource::Type type, ResourceRequest& request, const String& charset, ResourceLoadPriority priority, bool forPreload)    //SOMC KK PATCH Patches_w16.5
{
    KURL url = request.url();    //SOMC KK PATCH Patches_w16.5
    ...
    case Load:
        resource = loadResource(type, request, charset, priority);    //SOMC KK PATCH Patches_w16.5
        break;
    case Reload:
...
CachedResource* CachedResourceLoader::revalidateResource(CachedResource* resource, ResourceLoadPriority priority)
{
    ...
    CachedResource* newResource = createResource(resource->type(), resource->resourceRequest(), resource->encoding());    //SOMC KK PATCH Patches_w16.5
...
//SOMC KK PATCH Patches_w16.5_end
    ...
    m_validatedURLs.add(request.url());    //SOMC KK PATCH Patches_w16.5
    return resource;
}
...
void CachedResourceLoader::preload(CachedResource::Type type, ResourceRequest& request, const String& charset, bool referencedFromBody)    //SOMC KK PATCH Patches_w16.5
{
    ...
    if (!hasRendering && !canBlockParser) {
        ...
        PendingPreload pendingPreload = { type, request, charset };    //SOMC KK PATCH Patches_w16.5
        m_pendingPreloads.append(pendingPreload);
        return;
    }
    requestPreload(type, request, charset);    //SOMC KK PATCH Patches_w16.5
}
...
void CachedResourceLoader::requestPreload(CachedResource::Type type, ResourceRequest& request, const String& charset)    //SOMC KK PATCH Patches_w16.5
{
    String encoding;
    if (type == CachedResource::Script || type == CachedResource::CSSStyleSheet)
        encoding = charset.isEmpty() ? m_document->charset() : charset;

    CachedResource* resource = requestResource(type, request, encoding, ResourceLoadPriorityUnresolved, true);    //SOMC KK PATCH Patches_w16.5
