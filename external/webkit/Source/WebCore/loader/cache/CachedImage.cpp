...
CachedImage::CachedImage(Image* image)
    : CachedResource(ResourceRequest(), ImageResource)    //SOMC KK PATCH Patches_w16.5
    , m_image(image)
    , m_decodedDataDeletionTimer(this, &CachedImage::decodedDataDeletionTimerFired)
    , m_shouldPaintBrokenImage(true)
    , m_autoLoadWasPreventedBySettings(false)
...
void CachedImage::load(CachedResourceLoader* cachedResourceLoader)
{
    ...
    if (!cachedResourceLoader || (cachedResourceLoader->autoLoadImages() && !cachedResourceLoader->shouldBlockNetworkImage(m_resourceRequest.url())))    //SOMC KK PATCH Patches_w16.5
#else
...
void CachedImage::checkShouldPaintBrokenImage()
{
    ...
    m_shouldPaintBrokenImage = frame->loader()->client()->shouldPaintBrokenImage(m_resourceRequest.url());    //SOMC KK PATCH Patches_w16.5
}
