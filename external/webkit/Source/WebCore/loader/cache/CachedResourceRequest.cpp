...
static ResourceRequest::TargetType cachedResourceTypeToTargetType(CachedResource::Type type)    //SOMC KK PATCH Patches_w16.5
{
...
PassRefPtr<CachedResourceRequest> CachedResourceRequest::load(CachedResourceLoader* cachedResourceLoader, CachedResource* resource, bool incremental, SecurityCheckPolicy securityCheck, bool sendResourceLoadCallbacks)
{
    ...
    ResourceRequest resourceRequest = resource->resourceRequest();    //SOMC KK PATCH Patches_w16.5
    resourceRequest.setTargetType(cachedResourceTypeToTargetType(resource->type()));    //SOMC KK PATCH Patches_w16.5
...
#if ENABLE(LINK_PREFETCH)
    if (resource->type() == CachedResource::LinkPrefetch || resource->type() == CachedResource::LinkPrerender || resource->type() == CachedResource::LinkSubresource)    //SOMC KK PATCH Patches_w16.5
    ...
    if (!loader || loader->reachedTerminalState()) {
        ...
        LOG(ResourceLoading, "Cannot start loading '%s'", resource->url().string().latin1().data());    //SOMC KK PATCH Patches_w16.5
...
void CachedResourceRequest::didFinishLoading(SubresourceLoader* loader, double)
{
    ...
    LOG(ResourceLoading, "Received '%s'.", m_resource->url().string().latin1().data());    //SOMC KK PATCH Patches_w16.5
...
void CachedResourceRequest::didFail(bool cancelled)
{
    ...
    LOG(ResourceLoading, "Failed to load '%s' (cancelled=%d).\n", m_resource->url().string().latin1().data(), cancelled);    //SOMC KK PATCH Patches_w16.5
