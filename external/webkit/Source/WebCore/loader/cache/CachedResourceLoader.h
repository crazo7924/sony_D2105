...
public:
...
#if ENABLE(XSLT)
    CachedXSLStyleSheet* requestXSLStyleSheet(ResourceRequest&);    //SOMC KK PATCH Patches_w16.5
#endif
#if ENABLE(LINK_PREFETCH)
    CachedResource* requestLinkResource(CachedResource::Type, ResourceRequest&, ResourceLoadPriority = ResourceLoadPriorityUnresolved);    //SOMC KK PATCH Patches_w16.5
#endif
    ...
    void preload(CachedResource::Type, ResourceRequest&, const String& charset, bool referencedFromBody);    //SOMC KK PATCH Patches_w16.5
    void checkForPendingPreloads();
    void printPreloadStats();
    
private:
    CachedResource* requestResource(CachedResource::Type, ResourceRequest&, const String& charset, ResourceLoadPriority = ResourceLoadPriorityUnresolved, bool isPreload = false);    //SOMC KK PATCH Patches_w16.5
    CachedResource* revalidateResource(CachedResource*, ResourceLoadPriority priority);
    CachedResource* loadResource(CachedResource::Type, ResourceRequest&, const String& charset, ResourceLoadPriority);    //SOMC KK PATCH Patches_w16.5
    void requestPreload(CachedResource::Type, ResourceRequest& url, const String& charset);    //SOMC KK PATCH Patches_w16.5
    ...
    struct PendingPreload {
        ...
        ResourceRequest m_request;    //SOMC KK PATCH Patches_w16.5
        String m_charset;
    };
