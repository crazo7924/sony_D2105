...
bool MemoryCache::add(CachedResource* resource)
{
    ...
    LOG(ResourceLoading, "MemoryCache::add Added '%s', resource %p\n", resource->url().string().latin1().data(), resource);    //SOMC KK PATCH Patches_w16.5
    return true;
}
...
{
    LOG(ResourceLoading, "Evicting resource %p for '%s' from cache", resource, resource->url().string().latin1().data());    //SOMC KK PATCH Patches_w16.5
