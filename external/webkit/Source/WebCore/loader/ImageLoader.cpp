...
#include "CrossOriginAccessControl.h"    //SOMC KK PATCH Patches_w16.5
#include "Document.h"
#include "Element.h"
#include "HTMLNames.h"
...
void ImageLoader::updateFromElement()
{
    ...
    if (!(attr.isNull() || (attr.isEmpty() && document->baseURI().isLocalFile()))) {
        ...
        if (m_loadManually) {
            ...
            newImage = new CachedImage(request);    //SOMC KK PATCH Patches_w16.5
            newImage->setLoading(true);
            newImage->setOwningCachedResourceLoader(document->cachedResourceLoader());
            document->cachedResourceLoader()->m_documentResources.set(newImage->url(), newImage);
            document->cachedResourceLoader()->setAutoLoadImages(autoLoadOtherImages);
        } else
            newImage = document->cachedResourceLoader()->requestImage(request);    //SOMC KK PATCH Patches_w16.5
